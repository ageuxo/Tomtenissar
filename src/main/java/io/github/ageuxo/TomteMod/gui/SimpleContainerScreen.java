...
//        pGuiGraphics.drawCenteredString(minecraft.font, pMouseX+","+pMouseY, pMouseX, pMouseY, 0xFFFFFF);
    ...
    protected void renderBg(GuiGraphics pGuiGraphics, float pPartialTick, int pMouseX, int pMouseY) {
        pGuiGraphics.blitSprite(CONTAINER_PLATE, backplateX, backplateY, backplateEndX - backplateX, backplateEndY - backplateY);
        if (this.menu.columns > 9){
            pGuiGraphics.blitSprite(CONTAINER_PLATE, containerSlotX - 4, containerSlotY - 4, SimpleContainerMenu.SLOT_SIZE * this.menu.columns + 8, SimpleContainerMenu.SLOT_SIZE + 7);
        }
    ...
    public void renderExtraSlot(GuiGraphics guiGraphics, Slot slot) {
        guiGraphics.blitSprite(SLOT_TEXTURE, slot.x + this.leftPos - 1, slot.y + this.topPos - 1,  SimpleContainerMenu.SLOT_SIZE, SimpleContainerMenu.SLOT_SIZE);
    }

    public void renderContainerSlots(GuiGraphics guiGraphics){
        guiGraphics.blitSprite(SLOT_TEXTURE, containerSlotX -1, containerSlotY -1,this.menu.columns * SimpleContainerMenu.SLOT_SIZE, this.menu.rows * SimpleContainerMenu.SLOT_SIZE);

    }

    public void renderInventorySlots(GuiGraphics guiGraphics){
        guiGraphics.blitSprite(SLOT_TEXTURE, inventorySlotX, inventorySlotY,9 * SimpleContainerMenu.SLOT_SIZE, 3 * SimpleContainerMenu.SLOT_SIZE);
        guiGraphics.blitSprite(SLOT_TEXTURE, inventorySlotX, inventorySlotY + (3 * SimpleContainerMenu.SLOT_SIZE) + 4,9 * SimpleContainerMenu.SLOT_SIZE, SimpleContainerMenu.SLOT_SIZE);
    }
}
